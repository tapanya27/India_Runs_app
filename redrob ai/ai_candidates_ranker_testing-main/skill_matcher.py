...
class SemanticSkillMatcher:
    ...
    def __init__(self, jd_skills: list[str], use_cross_encoder_validation: bool = True):
        # use_cross_encoder_validation=False skips Layer-4 CE validation. The
        # challenge ranker disables it because match() is called once per
        # retrieved candidate (~700/JD); a per-candidate CE call there would add
        # hundreds of unbatched model calls to the 5-minute ranking budget. The
        # SBERT probable-match threshold already gates these matches.
        self.use_ce_validation = use_cross_encoder_validation
    ...
    def match(self, candidate_skills: list[str]) -> list[dict]:
        ...
        if unmatched_jd and unmatched_c:
            ...
            if probable_indices and self.use_ce_validation:
                ce_pairs = [
