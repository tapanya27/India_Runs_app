# Compact local models — chosen for the challenge's CPU-only, ≤5-minute ranking
# budget (submission_spec §3). all-MiniLM-L6-v2 (384-dim) encodes ~15× faster on
# CPU than all-mpnet-base-v2 while retaining strong retrieval quality, which makes
# the one-time 100k index build and the per-JD ranking step practical on CPU.
...
SKILL_ALIASES: dict[str, str] = {
    ...
    "es": "elasticsearch",
    "hf": "huggingface",
    # NOTE: deliberately NO "cv" -> "computer vision" alias. "cv" is ambiguous
    # ("curriculum vitae") and the JD's disqualifier line ("...only CV, speech,
    # or robotics...") would otherwise inject "computer vision" into JD skills.
    "bert": "bert",
    "gpt": "gpt",
    "llm": "llm",
    "rag": "rag",
